import{M as k,i as $,N as D,u as l,a as S,e as u,b as n,A as g,t as N}from"./index-CsG9DGtL.js";const x="oppai_libby_speak";function R(){try{return localStorage.getItem(x)==="1"}catch{return!1}}function U(e){try{localStorage.setItem(x,e?"1":"0")}catch{}e||I(),window.dispatchEvent(new CustomEvent("oppai-libby-speak",{detail:{on:e}}))}let h=null,v=0;async function P(e=!1){if(!e&&h&&Date.now()-v<6e4)return h;try{const t=await fetch("/api/tts/status",{credentials:"same-origin"});if(!t.ok)throw new Error(String(t.status));h=await t.json(),v=Date.now()}catch{h=null}return h}const m=[];let c=null,y=!1,b=0;function C(e){const t=e.trim();return t?new Promise(i=>{m.push({text:t,done:i}),L()}):Promise.resolve()}function I(){var e;b++;for(const t of m.splice(0))t.done();c&&(c.pause(),c.src="",c=null);try{(e=window.speechSynthesis)==null||e.cancel()}catch{}}async function L(){if(!y){y=!0;try{for(;m.length;){const e=m.shift(),t=b;try{await z(e.text,t)}catch{}finally{e.done()}}}finally{y=!1}}}async function z(e,t){const i=await B(e);if(t===b){if(i){await E(i,t);return}await A(e,t)}}async function B(e){const t=await P();if(t&&!t.engine)return null;try{const i=await fetch("/api/tts/speak",{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:e})});return i.status===503?(h=null,null):i.ok?await i.blob():null}catch{return null}}function E(e,t){return new Promise(i=>{const r=URL.createObjectURL(e),a=new Audio(r);c=a;const s=()=>{c===a&&(c=null),URL.revokeObjectURL(r),i()};a.addEventListener("ended",s,{once:!0}),a.addEventListener("error",s,{once:!0}),a.play().catch(()=>{s()}),t!==b&&s()})}function A(e,t){const i=window.speechSynthesis;return!i||typeof SpeechSynthesisUtterance>"u"?Promise.resolve():new Promise(r=>{const a=new SpeechSynthesisUtterance(e),s=T(i.getVoices());if(s&&(a.voice=s),a.rate=1.02,a.pitch=1.05,a.onend=()=>r(),a.onerror=()=>r(),t!==b){r();return}i.speak(a)})}function T(e){const t=e.filter(a=>/^en/i.test(a.lang)),i=t.length?t:e,r=a=>{let s=0;const p=a.name.toLowerCase();return/natural|neural|premium|enhanced/.test(p)&&(s+=4),/female|woman|aria|jenny|samantha|zira|karen|moira|tessa|fiona|libby|sonia|ava|allison|susan|emma/.test(p)&&(s+=3),/google/.test(p)&&(s+=1),a.default&&(s+=1),s};return[...i].sort((a,s)=>r(s)-r(a))[0]}var _=Object.defineProperty,j=Object.getOwnPropertyDescriptor,d=(e,t,i,r)=>{for(var a=r>1?void 0:r?j(t,i):t,s=e.length-1,p;s>=0;s--)(p=e[s])&&(a=(r?p(t,i,a):p(a))||a);return r&&a&&_(t,i,a),a};let o=class extends S{constructor(){super(...arguments),this.embedded=!1,this.backgrounds=[],this.defaultID="",this.loading=!0,this.busy=!1,this.note="",this.bad=!1,this.editing=null,this.draftName="",this.draftTags="",this.version=Date.now()}connectedCallback(){super.connectedCallback(),this.reload()}async reload(){this.loading=!0;try{const e=await u.libbyBackgrounds();this.backgrounds=e.backgrounds??[],this.defaultID=e.default??""}catch(e){this.say(e.message,!0)}finally{this.loading=!1}}say(e,t=!1){this.note=e,this.bad=t}changed(){this.dispatchEvent(new CustomEvent("changed",{bubbles:!0,composed:!0}))}startNew(){this.editing="new",this.draftName="",this.draftTags=""}startEdit(e){this.editing=e.id,this.draftName=e.name,this.draftTags=e.tags.join(", ")}async save(e){const t=this.draftName.trim();if(!t){this.say("A background needs a name.",!0);return}this.busy=!0;try{await u.saveLibbyBackground({id:e,name:t,tags:this.draftTags.split(",").map(i=>i.trim()).filter(Boolean)}),this.editing=null,await this.reload(),this.changed(),this.say(e?`Saved ${t}.`:`Added ${t}. Give it a picture so she can go there.`)}catch(i){this.say(i.message,!0)}finally{this.busy=!1}}async uploadPicture(e,t){var a;const i=t.target,r=(a=i.files)==null?void 0:a[0];if(i.value="",!!r){this.busy=!0;try{const s=await new Promise((p,w)=>{const f=new FileReader;f.onload=()=>p(String(f.result)),f.onerror=()=>w(f.error),f.readAsDataURL(r)});await u.setLibbyBackgroundImage(e,s),this.version=Date.now(),await this.reload(),this.changed(),this.say("Picture saved.")}catch(s){this.say(s.message,!0)}finally{this.busy=!1}}}async makeDefault(e){this.busy=!0;try{const t=this.defaultID===e?"":e;await u.setLibbyDefaultBackground(t),this.defaultID=t,this.changed(),this.say(t?"She starts here now.":"No default room — she starts nowhere in particular.")}catch(t){this.say(t.message,!0)}finally{this.busy=!1}}async deleteBackground(e){if(confirm(`Delete ${e.name}?`)){this.busy=!0;try{await u.deleteLibbyBackground(e.id),await this.reload(),this.changed(),this.say(`Deleted ${e.name}.`)}catch(t){this.say(t.message,!0)}finally{this.busy=!1}}}renderEditor(e){return n`<div class="edit">
      <label>Name
